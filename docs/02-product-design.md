...
4. The console displays video metadata, playlist entries, formats, and subtitle tracks.
5. User selects entries, a format, and subtitle preferences.
6. User starts a download task.
7. The task queue shows progress, speed, state, errors, and completed file links.

## Main Interface Regions

- Header: product name, local mode indicator, backend health state.
- Analyze panel: URL input, Analyze action, risk copy.
...
- Risk notice: copyright, platform terms, account risk warnings, plus a note that only public videos are supported.
