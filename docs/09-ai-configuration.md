...
AI 总结能力由后端统一配置，前端不会接收或保存 AI 服务密钥。项目现在只把本地配置集中到根目录 `.env`，不再默认读取 `backend/config/ai.config.json`。
...
部署环境可以直接注入同名环境变量，环境变量优先级高于 `.env`。
...
默认情况下，本项目不会读取浏览器 Cookie。只有你显式设置下面任一 `.env` 配置时，后端才会把授权信息交给 yt-dlp：
...
```text
默认值 < 显式 AI_CONFIG_FILE 旧 JSON 配置 < 根目录 .env < shell 环境变量
```

`AI_CONFIG_FILE` 仅作为旧配置迁移兼容入口保留；新配置请写入根目录 `.env`。
